...
function buildYear(target, startYear, endYear) {

	let optoins = '<option selected>--</option>';

	for (let i = endYear; i >= startYear; i--) {
		optoins += '<option value="' + i + '">' + i + '</option>'
	}
	$("#" + target).empty();
	$("#" + target).append(optoins);
}

function buildCityOption() {
	getAllCities(function suucessFunc(data) {
		let optoins = '<option selected>نام شهر</option>';
		for (city of data) {
			optoins += '<option value="' + city.id + '">' + city.faName + '</option>'
		}

		$("#cityOption").empty();
		$("#cityOption").append(optoins);
...
function getProfileInformation() {
	...
	$.ajax({
		...
		success: function (response) {
			...
			$('#brithYear option[value=' + response.brithYear + ']').attr('selected', 'selected');
			$("#userPhone").val(response.phone)
			$('#cityOption option[value=' + (response.city == null ? 0 : response.city.id) + ']').attr('selected', 'selected');
			$("#facebookLink").val(response.facebookLink);
			$("#twitterLink").val(response.twitterLink);
			$("#instagramLinkOne").val(response.instagramLinkOne);
			...
			if (response.confirmed == null || response.confirmed == false) {
				$("#userConfirmed").css("display", "none");
				$("#userConfirmedNot").css("display", "inline-block");
			}
			else {
...
function changeSelectedValue(thisObj) {
	...
	if ($(thisObj).prop("checked")) {
		url = "backend/api/artist/addSelectedSample/" + $(thisObj).val();
	}
	else {
	...
	for (media of medias) {
		if (media.contentType != null && (media.contentType.includes("video"))) {
			videoDiv +=
				...
				'data-bs-toggle="modal" onclick="confirmToRemove(\'skill_video_' + media.id + '\',\'' + skillId + '\',\'' + media.id + '\')"></i>' +
				'<div class="row">' +
			...
			photoDiv +=
				...
				'style="color: rgb(255, 255, 255);  position: absolute;  z-index: 10000;"' +
				' data-bs-toggle="modal" onclick="confirmToRemove(\'skill_image_' + media.id + '\',\'' + skillId + '\',\'' + media.id + '\')"></i>' +
...
function updateQusstionAnser(questionId, have) {
	...
	$.ajax({
		...
		success: function (response) {
			...
			buidSkillMedia(response, skillId)
...
function updateBackground(mediaId) {
	...
	$.ajax({
		url: "backend/api/artist/update/background/" + mediaId,
		method: "PUT",
		headers: {
			Authorization: "Bearer " + access_token
		},
		success: function (response) {
			...
			settupBackgroundProfile(response.mediaUrl, null);
		},
		error: function (xhr, status, error) {
...
function updateProfile(mediaId) {
	...
	$.ajax({
		url: "backend/api/artist/update/profile/" + mediaId,
		method: "PUT",
		headers: {
			Authorization: "Bearer " + access_token
		},
...
function updateProfileForm(thisObj, backgroundMediaForm) {
...
function updatePersonINfo(tagObj) {
	...
	let data = {
		...
		"brithYear": $("#brithYear option:selected").val(),
		"phone": $("#userPhone").val(),
		"cityId": $("#cityOption option:selected").val()
	}
	$(tagObj).append('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>')

	$.ajax({
		url: "backend/api/artist/save/personalInfo",
		method: "PUT",
		headers: {
			Authorization: "Bearer " + access_token
		},
...
function updateSocial(tagObj) {
	...
	let data = {
		...
		"insta1": $("#instagramLinkOne").val(),
		"insta2": $("#instagramLinkTwo").val()
	}
	$(tagObj).append('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>')

	$.ajax({
		url: "backend/api/artist/save/social",
		method: "PUT",
		headers: {
			Authorization: "Bearer " + access_token
		},
...
function updateAboutMe(tagObj) {
	...
	$.ajax({
		url: "backend/api/artist/save/aboutMe",
		method: "PUT",
		headers: {
			Authorization: "Bearer " + access_token
		},
...
function confirmToRemove(targetTagId, skillId, mediaId) {
...
function confirmToRemoveQuestionMedia(targetTagId, answerId, mediaId) {

	$("#generalConfirmBtt").attr("onclick", "$('#" + targetTagId + "').remove();deleteMediaFromAnswer(" + answerId + ", " + mediaId + ");$('#generalWarning').modal('toggle');")
	$("#generalWarning").modal('toggle');

}


function uploadMedia(targetTagId, answerId, mediaId) {
...
function addMedia2Skill(skillId, mediaId) {
	...
	if (skillId == "" || skillId == undefined || skillId == null) {
		findMediaByIds(JSON.parse($("#mediasIdsId").val()), function successCallback(data) {
...
function updateMainSkill(thisOject) {

	skillId = $("#skillId").val();

	if (skillId == "") {
		alert("ابتدا سایر فیلدها را پر کنید بعد دکمه ذخیره را بفشارید سپس مهارت مربوطه را به عنوان مهارت اصلی تنظیم کنید!")
	}
	let access_token = getCookie("access_token");
	if (access_token == undefined || access_token == "" || access_token == null) {
		location.href = 'login.html';
	...
	$.ajax({
		url: "backend/api/skill/updateTheMainSkill/" + skillId + "/" + $(thisOject).is(":checked"),
		method: "PUT",
		headers: {
			Authorization: "Bearer " + access_token
		},
...
function updateExtraDescription(tagObj) {
	...
	$.ajax({
		url: "backend/api/artist/save/extraDesc",
		method: "PUT",
		headers: {
			Authorization: "Bearer " + access_token
		},
...
function loadSkill(skillId) {
	...
	$.ajax({
		...
		success: function (response) {
			$("#categorySelect").val((response.category == null ? "null" : response.category.id))
			// $('#categorySelect option[value=' + (response.category == null ? 0 : response.category.id) + ']').attr('selected', 'selected');
			fetchArtStyle((response.category == null ? 0 : response.category.id), (response.instrument == null ? null : response.instrument.id), (response.artStyle == null ? null : response.artStyle.id))
			$("#skillId").val(response.id)
			$("#workExperienceYears").val(response.workExperienceYears)
			if (response.isMainSkill == null || response.isMainSkill == false) {
			...
			if (response.informationQuestions == null || response.informationQuestions.length == 0) {
				if (response.category != null || response.informationQuestions == null) {
					buildTemplateQuestion(response.category.id)
				}
...
function fetchArtStyle(categoryId, instrumnetId, artStyleId) {
	...
	$.ajax({
		url: "backend/api/category/find/" + categoryId,
		method: "GET",
		headers: {
			Authorization: "Bearer " + access_token
		},
		success: function (response) {
			if (response.id == 2 || response.id == 1003 || response.id == 2003) {
				$('.instrumnetClass').css('display', 'none');
				$('.artStyleClass').css('display', 'inline-block');
				$.ajax({
					url: "backend/api/artStyle/findByCategory/" + categoryId,
					method: "GET",
					headers: {
						Authorization: "Bearer " + access_token
					},
					success: function (response) {
						let options = "<option value=null >انتخاب کنید</option>";

						for (artStyle of response) {
							options += "<option " + (artStyleId != null && artStyleId == artStyle.id ? "selected=\"selected\" " : "") + " value=" + artStyle.id + " >" + artStyle.faName + "</option>"
						}

						$("#artStyle").empty()
						$("#artStyle").append(options)
				...
				$('.instrumnetClass').css('display', 'inline-block');
				$('.artStyleClass').css('display', 'none');
				$('#artStyle').val('null');
				getAllInstrumnet(instrumnetId);
			}
			else {
				$('.instrumnetClass').css('display', 'none');
				$('.artStyleClass').css('display', 'none');
				$('#artStyle').val('null');
				$('#instrumentss').val('null');
			}
...
function saveSkill(isNew) {
	...
	let mediaIds = JSON.parse($("#mediasIdsId").val());
